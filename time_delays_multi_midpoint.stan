...
model {
  ...
  for (n in 1:N){
    target += lognormal_lpdf((b[n] - a[n] ) | mu, sigma) - log(0.1 * exp(lognormal_lcdf(upper_bound - a_minus[n] + 0.05 | mu,sigma)) +
            0.1 * exp(lognormal_lcdf(upper_bound - a_minus[n] + 0.15 | mu,sigma)) +
            0.1 * exp(lognormal_lcdf(upper_bound - a_minus[n] + 0.25 | mu,sigma)) +
            0.1 * exp(lognormal_lcdf(upper_bound - a_minus[n] + 0.35 | mu,sigma)) +
            0.1 * exp(lognormal_lcdf(upper_bound - a_minus[n] + 0.45 | mu,sigma)) +
            0.1 * exp(lognormal_lcdf(upper_bound - a_minus[n] + 0.55 | mu,sigma)) +
            0.1 * exp(lognormal_lcdf(upper_bound - a_minus[n] + 0.65 | mu,sigma)) +
            0.1 * exp(lognormal_lcdf(upper_bound - a_minus[n] + 0.75 | mu,sigma)) +
            0.1 * exp(lognormal_lcdf(upper_bound - a_minus[n] + 0.85 | mu,sigma)) +
            0.1 * exp(lognormal_lcdf(upper_bound - a_minus[n] + 0.95 | mu,sigma)));
  }
}
